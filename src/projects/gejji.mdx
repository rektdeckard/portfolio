...
Given that the host code was already written in Rust, and seeing the opportunity for a challenge, I decided to rewrite the microcontroller code in Rust as well. Using [espflash](https://github.com/esp-rs/espflash) and [this lovely guide by Scott Mabin](https://mabez.dev/blog/posts/esp-rust-ecosystem), I was able to compile Rust for the ESP32 board. But while ultimately possible, I found few benefits — and a number of difficulties around the tooling needed to get it working. I eventually went back to C++ for the microcontroller code, but hope to see progress in this space in the future.
