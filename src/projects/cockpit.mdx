...
```js
function generateMapURL(attrs) {
  let url = new URL(BASE_URL);
  let params = new URLSearchParams({
    mapType: "roadmap",
    lat: `${attrs.lat}`,
    lon: `${attrs.lon}`,
    size: Math.round(attrs.height ?? 600).toString(),
    scale: `${attrs.scale ?? 1}`,
    zoom: `${attrs.zoom ?? 10}`,
    key: API_KEY,
  });
  params.append(
    "style",
    `feature:all|element:labels|visibility:${
      attrs.labels ? "simplified" : "off"
    }`,
  );
  params.append(
    "style",
    `feature:administrative|element:labels|visibility:${
      attrs.labels && attrs.administrtativeLabels ? "simplified" : "off"
